...
int main() {
    ...
    auto updateFilter = [&]() {
        ...
        for (const char& c : filter) *currentFilter.family += c;
        filtered.clear();
    ...
    h.on(
        vt::Handler::Event::KEY,
        (vt::Handler::callback_bool)[&](vt::Handler & handler)->bool {
            if (mode != Mode::FILTER) return true;
    ...
    h.on(vt::Handler::Event::DRAW, [&](vt::Handler& handler) {
        ...
        vt::eraseToEnd();
        for (const char& c : filter) vt::putc(c);
        vt::lineDown();
    });
    ...
    h.on(vt::Handler::Event::KEY, [&](vt::Handler& handler) {
        ...
        vt::byte key = handler.getLastKey();
        if (key == 'j') {
            if (selected < filtered.size()) ++selected;
        }
        if (key == 'k') {
        ...
        if (key == 'H') selected = 0;
        if (key == '\r') handler.quit();  // selected
    });
